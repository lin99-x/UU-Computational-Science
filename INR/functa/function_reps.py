...
def get_coordinate_grid(res: int, centered: bool = True) -> Array:
  """Returns a normalized coordinate grid for a res by res by res sized image.
  ...
  Returns:
    Jnp array of shape (depth, height, width, 3).
  ...
  # Array will have shape (depth, height, width, 3)
  return jnp.stack(
      jnp.meshgrid(coords_one_dim, coords_one_dim, coords_one_dim, indexing='ij'), axis=-1)
