...
def get_config():
  ...
  # Training config
  per_device_batch_size = 1 if exp.dataset.type == 'volume' else 16
  ...
  exp.evaluation.num_examples = 10 if exp.dataset.type == 'volume' else -1
...
class Experiment(experiment.AbstractExperiment):
  ...
  def _build_train_input(self, batch_size: int) -> Generator[Array, None, None]:
    """See base class."""
    if self.config.dataset.type == 'volume':
