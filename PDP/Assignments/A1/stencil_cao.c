...
int main(int argc, char **argv) {
	...
	for (int s=0; s<num_steps; s++) {
		...
		// it could be faster if the non-blocking version is correctly used
		if (size != 1){
			if (rank==0){
				MPI_Send(local_input+EXTENT, EXTENT, MPI_DOUBLE, size-1, 000, MPI_COMM_WORLD);
				MPI_Recv(local_input+chunk+EXTENT, EXTENT, MPI_DOUBLE, rank+1, 000, MPI_COMM_WORLD, &status);
				MPI_Send(local_input+chunk, EXTENT, MPI_DOUBLE, rank+1, 111, MPI_COMM_WORLD);
				...
				MPI_Send(local_input+EXTENT, EXTENT, MPI_DOUBLE, rank-1, 000, MPI_COMM_WORLD);
	...
	// printf("%f\n", my_execution_time); //this doesn't fulfill the requirement. help me finish this
	if (rank==0){
		// Write result
