...
int main(int argc, char *argv[]) {
    ...
    // send local B to right process and receive from left process using MPI_Isend and MPI_Irecv
    MPI_Isend(local_B_recv, chunk_size * n, MPI_DOUBLE, right, step, MPI_COMM_WORLD, &send_request);
    MPI_Irecv(local_B, chunk_size * n, MPI_DOUBLE, left, step, MPI_COMM_WORLD, &recv_request);

    // Wait for communication to complete
    MPI_Wait(&send_request, &status);
    MPI_Wait(&recv_request, &status);

    // Swap local_B and local_B_recv
    double *temp = local_B;
