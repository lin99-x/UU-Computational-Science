...
int main(int argc, char **argv)
{
    ...
    if (size > 1) {
        ...
        // double sort_end = MPI_Wtime() - sort_start;
        // printf("I am process %d, my run time is %lf\n", rank, run_time);
        MPI_Reduce(&run_time, &max_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
        // MPI_Reduce(&sort_time, &sort_time, 1, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
    } 
    
    else {
        final_array = local_array;
        // printf("%f", runtime_for_oneprocessor - time_for_oneprocessor);
        /*
        for (int i=0; i<N; i++){
            printf("%lf ",final_array[i]);
        }
    ...
    if (rank == 0) {
        ...
        // for (int i = 0; i < N; i++) {
        //     fprintf(output, "%d ", (int)final_array[i]);
        // }
