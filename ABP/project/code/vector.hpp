                              ...
                              Number *destination)
{
  ...
  const int idx = threadIdx.x + blockIdx.x * blockDim.x;
  if (idx < N)
    destination[idx] = destination[idx] * scalar1 + source[idx] * scalar2;
}
...
{
  __shared__ Number sdata[block_size];    // shared among threads within a CUDA block
...
public:
  ...
  // create a new vector that is an exact copy of an existing vector
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        AssertCuda(cudaMemcpy(data,
                              ...
                              cudaMemcpyDeviceToDevice));         // from 'other' to data
      }
    else
      {
...
#pragma omp parallel for simd
  ...
  const Number &operator()(const std::size_t index) const   // can read but not modify it
  {
    return data[index];
  }
...
#pragma omp parallel for simd
  ...
  {
    assert_size(other);     // check the local_size of two vectors
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        vector_update<Number><<<n_blocks, block_size>>>(local_size, my_scalar, other_scalar, other.data, data);
#endif
      }
    else
      {
  ...
  Vector copy_to_device()
  {
    if (memory_space == MemorySpace::CUDA)
    ...
    else
      {
        ...
        AssertCuda(cudaMemcpy(other.data, this->data, local_size * sizeof(Number), cudaMemcpyHostToDevice));
        return other;
      }
  }
    ...
    if (memory_space == MemorySpace::CUDA)
      {
        ...
        AssertCuda(cudaMemcpy(other.data, this->data, local_size * sizeof(Number), cudaMemcpyDeviceToHost));
        return other;
      }
    else
      {
...
private:
  ...
  Number *    data;
  std::size_t local_size;       // the size of the vector owned by the current processor 
  std::size_t global_size;      // total number of elements
  ...
  void resize(const std::size_t local_size)
  {
    ...
    this->operator=(0.);      // set all elements of the vector to zero
  }
