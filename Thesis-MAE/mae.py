...
class MAE(nn.Module):
    ...
    def patchify(self, img):
        ...
        x = img.reshape(shape=(img.shape[0], self.in_chans, h, p, w, p))
        x = torch.einsum('nchpwq->nhwpqc', x)
        x = x.reshape(shape=(img.shape[0], h*w, p**2 * self.in_chans))
        return x
    ...
    def unpatchify(self, x):
        ...
        img = x.reshape(shape=(x.shape[0], self.in_chans, h*p, w*p))
        return img
